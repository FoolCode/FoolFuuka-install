	...
	<article id="<?= $p->num ?>" class="thread doc_id_<?= $p->doc_id ?>">
		<header>
			<div class="post_data">
				...
				<span class="post_number"><a href="<?= Uri::create(Radix::get_selected()->shortname . '/thread/' . $p->num) . '#' . $p->num ?>" data-function="highlight" data-post="<?= $p->num ?>">No.</a><a href="<?= Uri::create(Radix::get_selected()->shortname . '/thread/' . $p->num) . '#q' . $p->num ?>" data-function="quote" data-post="<?= $p->num ?>"><?= $p->num ?></a></span>
				<span class="post_controls"><a href="<?= Uri::create(Radix::get_selected()->shortname . '/thread/' . $p->num) ?>" class="btnr parent"><?= __('View') ?></a><a href="<?= Uri::create(Radix::get_selected()->shortname . '/thread/' . $p->num) . '#reply' ?>" class="btnr parent"><?= __('Reply') ?></a><?= (isset($p->count_all) && $p->count_all > 50) ? '<a href="' . Uri::create(Radix::get_selected()->shortname . '/last50/' . $p->num) . '" class="btnr parent">' . __('Last 50') . '</a>' : '' ?><?php if (Radix::get_selected()->archive == 1) : ?><a href="http://boards.4chan.org/<?= Radix::get_selected()->shortname . '/res/' . $p->num ?>" class="btnr parent"><?= __('Original') ?></a><?php endif; ?><a href="<?= Uri::create(Radix::get_selected()->shortname . '/report/' . $p->doc_id) ?>" class="btnr parent" data-function="report" data-post="<?= $p->doc_id ?>" data-post-id="<?= $p->num ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true"><?= __('Report') ?></a><?php if (Auth::has_access('maccess.mod')) : ?><a href="<?= Uri::create(Radix::get_selected()->shortname . '/delete/' . $p->doc_id) ?>" class="btnr parent" data-function="delete" data-post="<?= $p->doc_id ?>" data-post-id="<?= $p->num ?>" data-controls-modal="post_tools_modal" data-backdrop="true" data-keyboard="true"><?= __('Delete') ?></a><?php endif; ?></span>
			</div>
		</header>
		<div class="thread_image_box">
			<?php if ($p->media_status != 'available') :?>
				...
				<?php endif; ?>
			<?php else: ?>
			<a href="<?= Uri::create(Radix::get_selected()->shortname . '/thread/' . $p->num) ?>" data-backlink="<?= $p->num ?>" rel="noreferrer" target="_blank" class="thread_image_link"<?= ($p->media_link)?' data-expand="true"':'' ?>>
				<?php if(!Auth::has_access('maccess.mod') && !Radix::get_selected()->transparent_spoiler && $p->spoiler) :?>
			...
			<?php if ($p->media_status == 'banned') : ?>
				<div class="post_file_controls">
					<a href="<?= ($p->media_link) ? $p->media_link : $p->remote_media_link ?>" class="btnr" target="_blank">Full</a><a href="<?= Uri::create(Radix::get_selected()->shortname . '/search/image/' . urlencode(substr($p->media_hash, 0, -2))) ?>" class="btnr parent"><?= __('View Same') ?></a><a target="_blank" href="http://iqdb.org/?url=<?= $p->thumb_link ?>" class="btnr parent">iqdb</a><a target="_blank" href="http://saucenao.com/search.php?url=<?= $p->thumb_link ?>" class="btnr parent">SauceNAO</a><a target="_blank" href="http://google.com/searchbyimage?image_url=<?= $p->thumb_link ?>" class="btnr parent">Google</a>
				</div>
			<?php endif; ?>
		</div>
